...
def find_iso(iso: pycdlib.PyCdlib, target_file: str, path_type: str):
    ...
    for parent_path, _, files in iso.walk(**{path_type: "/"}):
        ...
        dr = next(
            (
                d
                for d in children
                if d.file_identifier().decode("utf-8").rstrip(";1").upper()
                == target_upper
            ),
            None,
        )
...
if __name__ == "__main__":
    find_and_print_file("output.iso", "file1.txt")
